...
class _LoginCardState extends State<_LoginCard> with TickerProviderStateMixin {
  ...
  final _resturantNameFocusNode = FocusNode();
  ...
  TextEditingController _resturantNameController;
  ...
  Future<bool> _submit() async {
    ...
    if (auth.isLogin) {
      ...
      error = await auth.onSignup(LoginData(
          ...
          password: auth.password,
          userName: auth.name,
          locationId: SharedPref.myLocation.id,
          resturantName: auth.resturantName));
    }
  ...
  Widget _buildNameField(double width, LoginMessages messages, Auth auth) {
    return AnimatedTextFormField(
      ...
      onFieldSubmitted: (value) {
        FocusScope.of(context).requestFocus(_resturantNameFocusNode);
      },
      validator: auth.isSignup
          ? (value) {
              if (value.length == 0) {
                return "يجب إدخال اسمك";
              }
              return null;
            }
          : (value) => null,
  ...
  Widget _buildResturantNameField(
      double width, LoginMessages messages, Auth auth) {
    return AnimatedTextFormField(
      ...
      loadingController: _loadingController,
      interval: _nameTextFieldLoadingAnimationInterval,
      labelText: messages.resturantNameHint,
      ...
      validator: auth.isSignup
          ? (value) {
              if (value.length == 0) {
                return "يجب إدخال اسم مطعمك";
              }
              return null;
            }
          : (value) => null,
      onSaved: (value) => auth.resturantName = value,
    );
  }

  Widget _buildLocationField(double width, LoginMessages messages, Auth auth) {
    return Container(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: DropdownButton(
                ...
                hint: Text('المنطقة'),
                isExpanded: true,
                items: SharedPref.items,
                value: SharedPref.myLocation,
                onChanged: (location) {
                  setState(() {
                    SharedPref.myLocation = location;
                  });
                },
              ),
            ),
      ...
      child: Column(
        children: [
          ...
          ExpandableContainer(
            ...
            onExpandCompleted: () => _postSwitchAuthController.forward(),
            child: _buildResturantNameField(textFieldWidth, messages, auth),
          ),
          ExpandableContainer(
            backgroundColor: theme.accentColor,
            controller: _switchAuthController,
            ...
            onExpandCompleted: () => _postSwitchAuthController.forward(),
            child: _buildLocationField(textFieldWidth, messages, auth),
          ),
          Container(
